...
@st.cache_data
def load_data():
    # References the production file provided
...
# --- PAGE 2: EMPLOYEE SEARCH ---
    ...
    with s_col1:
        search_id = st.text_input("Enter 6-Digit EMPID:", placeholder="Search...", label_visibility="collapsed")
    with s_col2:
            ...
            with insight_col:
                ...
                with st.container(border=True):
                    if emp['Risk_Level'] == 'High':
                        st.write("• **ER Manager Physical Intervention**: Urgent in-person visit to signal genuine organizational care.")
                        ...
                        st.write("• **Future Goals Check-ins**: Bi-annual conversations to co-create development roadmaps.")
                        st.write("• **OJP Internal Leads**: Assign as SMEs on cross-functional projects to keep work stimulating.")
                        st.write("• **Appreciation Programs**: Periodic nomination for 'Star Performer' awards.")
        else:
